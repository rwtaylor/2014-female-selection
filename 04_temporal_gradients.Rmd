...
seCoefMCMC <- function(x){
  mod <- lm(rel_ars ~ aggression_f + activity_f + docility_f, data = x)
  mod.sd <-  lm(rel_ars ~ aggression_sy + activity_sy + docility_sy, data = x)
  res <- c(as.list(coef(mod)[-1]), as.list(coef(mod.sd)[-1]))
  res$Year <- x$Year[1]
  res$itt <- x$itt[1]
  # Calculate standardized selection gradients
  res$docility_m    <- res$docility_f * mean(x$docility, na.rm = TRUE)
...
getCred <- function(x, sig = 0.05){
  require(MCMCglmm)
  mcmc_data <- x %>% ungroup() %>% select(aggression_fm, activity_fm, docility_fm, aggression_sy, activity_sy, docility_sy, docility_m) %>% mcmc(.)
...
```{r seCoeff}
# Calculate standardized selection gradients
res$docility_fm    <- coefs["docility"] * mean(x$docility, na.rm = TRUE)
...
# Don't report confidence intervals?
seCoeffLmer <- function(x){
  model <- lm(rel_ars ~ aggression_fm + activity_fm + docility_fm, data = x)
  model <- lm(rel_ars ~ aggression_fm + activity_fm + docility_fm, data = x)
