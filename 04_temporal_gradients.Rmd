...
seCoefMCMC <- function(x){
  ...
  res$docility_m    <- res$docility * mean(x$docility, na.rm = TRUE)
  res$aggression_sd <- res$aggression * sd(x$aggression, na.rm = TRUE)
  res$activity_sd   <- res$activity * sd(x$activity, na.rm = TRUE)
  res$docility_sd   <- res$docility * sd(x$docility, na.rm = TRUE)
...
getCred <- function(x, sig = 0.05){
  ...
  mcmc_data <- x %>%
    ungroup() %>%
    select(aggression, activity, docility, docility_m, aggression_sd,
      activity_sd, docility_sd) %>%
    mcmc(.)
  pm  <- posterior.mode(mcmc_data)
  int <- HPDinterval(mcmc_data, prob = 1 - sig)
  tbl_df(data.frame(
    Year      = x$Year[1],
    variable  = c("Aggression", "Activity", "Docility", "Docility",
     "Aggression", "Activity", "Docility"),
     standardization = c("None", "None", "None", "Mean", "SD", "SD", "SD"),
    post_mode = pm,
    lower     = int[ ,"lower"],
    upper     = int[ ,"upper"],
...
sgt_agg <- filter(sgt, variable == "Aggression", standardization == "SD")
sgt_act <- filter(sgt, variable == "Activity", standardization == "SD")
sgt_doc <- filter(sgt, variable == "Docility", standardization == "SD")

sgt_ns_agg <- filter(sgt, variable == "Aggression", standardization == "None")
sgt_ns_act <- filter(sgt, variable == "Activity", standardization == "None")
sgt_ns_doc <- filter(sgt, variable == "Docility", standardization == "None")

sgt_m_doc <- filter(sgt, variable == "Docility", standardization == "Mean")
...
pandoc.table(
  data.frame(Year = N$Year, N = N[ ,2], Docility = sgt_m_doc$coef
  ),
...
```{r sel_grads_mcmc_plot_Mean, fig.cap="Mean Standardized Selection Gradients"}
load("data/analyses_data/sel_grads_mcmc_post.RData")
library(ggplot2)
library(dplyr)
...
p <- ggplot(data = filter(sel_grads_mcmc, standardization == "Mean"),
  aes(x = Year, y = post_mode, group = variable)
)
p <- p + geom_hline(yintercept = 0, size = 0.25) # Line at y = 0
p <- p + geom_errorbar(aes(ymax = upper, ymin = lower),
...
pdf(file = "figure/sel_grads_mcmc_plot_mean.pdf", width = 4.33, height = 3)
p_sel_grad_MCMC
dev.off()
p_sel_grad_MCMC
```
...
```{r seCoeff}
# Calculate standardized selection gradients
res$docility_m    <- coefs["docility"] * mean(x$docility, na.rm = TRUE)
res$aggression_sd <- coefs["aggression"] * sd(x$aggression, na.rm = TRUE)
res$activity_sd   <- coefs["activity"] * sd(x$activity, na.rm = TRUE)
res$docility_sd   <- coefs["docility"] * sd(x$docility, na.rm = TRUE)

x <- fit_raneff_data %>% filter(type == "blup" & Year == 2003)

### RYAN START HERE
# Seems safest to just calc. standardized traits... then run a model for each
# But how to do for docility mean standardized?
# 
seCoeffLmer <- function(x){
  model <- lm(rel_ars ~ aggression_m + activity_m + docility_m, data = x)
  mod.coefs <- coef(model)[-1]
  sim.coefs <- data.frame(coef(sim(model))[ ,-1]) ## Simulated coefficients for calculating uncertainty.
  names(sim.coefs) <- names(mod.coefs)
  sim.CI <- apply(sim.coefs, 2, quantile, prob = c(0.025, 0.975)) #0.95 conf. int.
  # Calculate standardized coefficients (mean stan. only for docil)
  sd.traits <- c(sd(x$aggression, na.rm = TRUE),
                 sd(x$activity, na.rm = TRUE),
                 sd(x$docility, na.rm = TRUE)
                 )
  sd.coefs <- mod.coefs * sd.traits
  sd.matrix <- matrix(c(sd.traits, sd.traits), ncol = 3, byrow = TRUE)
  sim.sd.coefs <- sim.coefs * sd.traits
  sim.sd.CI <- apply(sim.sd.coefs, 2, quantile, prob = c(0.025, 0.975)) #0.95 conf. int.
  2.5%      -0.990628 -0.05080394  -0.7053028
  97.5%      0.727805  1.94957088   0.5780641
  
  
  docil.mean.coef <- mod.coefs["docility"] * mean(x$docility, na.rm = TRUE)
  out <- data.frame(
    standardization = c("None","None","None","SD","SD","SD","Mean"),
    Year = as.numeric(as.character(x$Year[1:7])),
    variable = c("Aggression", "Activity", "Docility", 
      "Aggression", "Activity", "Docility", "Docility"),
    coefficients = c(mod.coefs, sd.coefs, docil.mean.coef),
    upper = coefs + t_0.05,
    lower = coefs - t_0.05,
