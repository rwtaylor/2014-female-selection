...
seCoefMCMC <- function(x){
  mod <- lm(rel_ars ~ aggression_m + activity_m + docility_m, data = x)
  ...
  res$docility_m    <- res$docility_m * mean(x$docility, na.rm = TRUE)
...
getCred <- function(x, sig = 0.05){
  require(MCMCglmm)
  mcmc_data <- x %>% ungroup() %>% select(aggression, activity, docility, aggression_sy, activity_sy, docility_sy, docility_m) %>% mcmc(.)
