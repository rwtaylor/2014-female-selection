...
mis_sub_data <- foreach(i = 1:100, .combine = 'rbind') %do% {
  mis_data %>%
  group_by(ID) %>%
  do(sample_n(., 1)) %>%
  mutate(itt = i)  
}
save(mis_data, mis_sub_data, file = "data/analysis_steps/mis_sub.Rdata")
```
### Bartlett's test & KMO

```{r bartlett}
load("data/analysis_steps/mis_sub.Rdata")
library(psych)
...
mis_pca <- gather(data.frame(mis_pca), key = "trait", value = "loading")
...
of_sub_data <- foreach(i = 1:100, .combine = 'rbind') %do% {
  of_data %>%
  group_by(ID) %>%
  do(sample_n(., 1)) %>%
  mutate(itt = i)  
}
save(of_data, of_sub_data, file = "data/analysis_steps/of_sub_data.RData")
```

### Bartlett's test & KMO

```{r of_bartlett}
load("data/analysis_steps/of_sub_data")
library(psych)
...
p <- ggplot(gather(kmo_of, key = "behav", value = "KMO"), aes(x = KMO))
...
of_pca <- gather(data.frame(of_pca), key = "trait", value = "loading")
...
save(mis_pca_summary, of_pca_summary,
  file = "data/analysis_steps/pca.RData")
```
