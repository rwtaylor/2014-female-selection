...
save(sel_grads_mcmc_comp, competition_year,
  file = "data/analysis_steps/sel_grads_mcmc_comp.RData")
```

```{r calc_correlations}
load("data/analysis_steps/sel_grads_mcmc_comp.RData")
...
save(ars_linear_comp_posterior,
  file = "data/analysis_steps/ars_linear_comp_posterior.RData")
```

```{r format_ac}
load("data/analysis_steps/ars_linear_comp_posterior.RData")
library(MCMCglmm)
library(lme4)
library(data.table)
...
save(nonlinear_mcmc, file = "data/analysis_steps/nonlinear_mcmc_models.RData")
```

```{r format_for_tables}
load("data/analysis_steps/nonlinear_mcmc_models.RData")
...
save(sel_grads_blup_competition, competition_year,
  file = "data/analysis_steps/sel_grads_blup_competition.RData")
```

```{r comp_sg_plot_blup, fig.width=4.33, fig.height=4.5, fig.cap = ""}
load("data/analysis_steps/sel_grads_blup_competition.RData")
...
save(ars_linear_model, kpd_linear_model, ows_linear_model,
  file = "data/analysis_steps/ars_linear_blup_models.RData")
```
...
```{r format_ac_blup}
load("data/analysis_steps/ars_linear_blup_models.RData")
library(lme4)
...
save(ars_nl_model,kpd_nl_model, ows_nl_model,
  file = "data/analysis_steps/nl.blup_models.RData")
...
load("data/analysis_steps/nl.blup_models.RData")
